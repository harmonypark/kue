...
exports.out = function(req, res){
  var id = req.params.id, stream, out, file;
  Job.out(id, function(err, out){
    if (err) return res.send({ error: err.message }, 404);
    out = out || {};
    file = out.file;
    if(file){
...
exports.createJob = function(req, res) {
  var body = req.body, validation, job, options;
  ...
  job.save(function(err) {
    ...
    res.send({ message: 'job accepted', id: job.id }, 202);
  });
