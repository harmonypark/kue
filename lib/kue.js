...
Queue.prototype.process = function(type, n, fn, schema){
  var self = this;
...
/**
 * Register schema for jobs with the given `type`.
 *
 * @param {String} type
 * @param {Object} schema
 * @api public
...
Queue.prototype.registerSchema = function(type, schema){
  if(type && schema){
    schemas[type] = schema;
  }
  return this;
...
/**
 * Validate obj againts schema with the given `type`.
 *
 * @param {String} type
 * @param {Object} json
 * @api public
...
Queue.prototype.validate = function(type, json){
  if(schemas.hasOwnProperty(type)){
    return jsvEnv.validate(json, schemas[type]);
  } else {
    return;
  }
};
