...
/**
 * Get job output with `id` and callback `fn(err, job)`.
 *
 * @param {Number} id
 * @param {Function} fn
 * @api public
...
exports.out = function(id, fn){
  var client = redis.client()
    , out = {};

  client.hgetall('q:job:' + id, function(err, hash){
    if (err) return fn(err);
    if (!hash) return fn(new Error('job "' + job.id + '"output doesnt exist'));
    if (!hash.type) return fn();
    try {
      if (hash.out) out = JSON.parse(hash.out);
      fn(err, out);
    } catch (err) {
      fn(err);
    }
...
/**
 * Initialize a new `Job` with the given `type`, `data`.
 *
 * @param {String} type
 * @param {Object} data
 * @api public
...
Job.prototype.output = function(val, fn){
  var json;
  // serialize json data
  try {
  ...
  this.client.hset('q:job:' + this.id, 'out', json, fn || noop);
  return this;
};
