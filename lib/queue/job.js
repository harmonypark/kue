...
exports.get = function(id, fn){
  ...
  client.hgetall('q:job:' + job.id, function(err, hash){
    ...
    try {
      if (hash.input) job.input = JSON.parse(hash.input);
      if (hash.output) job.output = JSON.parse(hash.output);
      fn(err, job);
    } catch (err) {
      fn(err);
    }
...
/**
 * Initialize a new `Job` with the given `type`, `input`.
 *
 * @param {String} type
 * @param {Object} input
 * @api public
...
Job.prototype.toJSON = function(){
  return {
    ...
    , progress: parseInt(this._progress) || 0
    , state: this._state
    , error: this._error
...
Job.prototype.output = function(val, fn){
  ...
  this.client.hset('q:job:' + this.id, 'output', json, fn || noop);
  return this;
};
...
Job.prototype.update = function(fn){
  ...
  // serialize json
  try {
    json = JSON.stringify(this.input);
  } catch (err) {
    return fn(err);
  }
