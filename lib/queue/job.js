...
exports.get = function(id, fn){
  ...
  client.hgetall('q:job:' + job.id, function(err, hash){
    ...
    try {
      ...
      if (hash.results) job.results = JSON.parse(hash.results);
      fn(err, job);
    } catch (err) {
      fn(err);
    }
...
/**
 * Initialize a new `Job` with the given `type`, `data` and intermediate `results`.
 *
 * @param {String} type
 * @param {Object} data
...
function Job(type, data, results) {
  this.type = type;
  this.data = data || {};
...
Job.prototype.output = function(val, fn){
  ...
  this.client.hset('q:job:' + this.id, 'results', json, fn || noop);
  return this;
};
